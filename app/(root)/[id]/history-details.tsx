...
const HistoryDetails: React.FC = () => {
  ...
  const {
    ...
    loading: carLoading,
    error: carError,
  } = useFetch<{ data: Car }>(`/(api)/car/${query}`, {
    method: "GET",
  });
  ...
  // 1. Handler to cancel (refund) the booking
  const onCancel = async () => {
    if (!id) return;
    try {
      // Build refund payload
      const refundPayload = {
        amount: refundAmount, // from calculateCancellationDetails
        remarks: "Cancel booking",
        first_name: user?.firstName,
        last_name: user?.lastName,
      };
      ...
      const refundResponse = await initiateRefund(id, refundPayload);

      // Check for success status (200) in your refundResponses array
      const status = refundResponse.refundResponses?.[0]?.status;
      if (status === "200") {
        setDidCancel(true); // flag that we’ve just cancelled
        setShowSuccess(true); // show success modal
      } else {
        // any non-200 we consider an error
        console.error("Refund failed:", refundResponse);
        setShowError(true);
      }
    } catch (err) {
      console.error("Cancelling booking error:", err);
      setShowError(true);
    }
  };

  // 2. Handler to complete a pending payment
  const onPay = async () => {
    ...
    try {
      ...
      const payResponse = await initiatePayment(paymentPayload);
  ...
  useEffect(() => {
    if (callback === "true" || completePayment === "true") {
      if (completePayment === "true" && booking?.paymentType === "reserve") {
        setShowError(true);
      } else {
        setShowSuccess(true);
      }
