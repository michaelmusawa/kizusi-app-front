...
const BookDetails = () => {
  ...
  const { data: carResponse } = useFetch<{ data: Car }>(
    `/(api)/car/${id || ""}`,
    {
      method: "GET",
    }
  ...
  }, [paymentType, rideAmount, addonsAmount]);
  ...
  const handlePayment = async () => {
    ...
    const paymentData = {
      ...
      email: returnedUser?.email ?? user?.primaryEmailAddress?.emailAddress,
      phoneNumber: returnedUser?.phone ?? user?.primaryPhoneNumber,
      image: returnedUser?.image ?? user?.imageUrl,
      carId: Number(id),
      reference,
      userId: user?.id,
      bookingDate: locationStore.date,
      bookingEndDate: locationStore.endDate,
      departureLatitude: locationStore.departureLatitude,
      departureLongitude: locationStore.departureLongitude,
      destinationLatitude: locationStore.destinationLatitude,
      destinationLongitude: locationStore.destinationLongitude,
      departure: locationStore.departureAddress,
      destination: locationStore.destinationAddress,
      bookType: locationStore.bookType,
      paymentType,
      addons,
      description: "Car Rental Payment",
      callbackUrl: Linking.createURL(
    ...
    if (result.redirect_url) {
      router.push(
        // eslint-disable-next-line prettier/prettier
        `/(root)/paymentWebView?callbackUrl=${result.redirect_url}`
      );
    }
  };
